            ...
            <n-card hoverable>
                ...
                <n-ul>
                    ...
                    <n-tooltip trigger="hover">
                        ...
                        任何与JumpFrp映射相关的问题，都可以联系技术QQ：242247494申请手把手教学！
                    </n-tooltip>
                </n-ul>
                <template #action>
                    <n-flex justify="space-between">
...
const checkTradeStatus = () => {
    if (route.query.trade_status === 'TRADE_SUCCESS') {
        dialog.success({
            title: '会员购买成功！',
            content: `您已成功购买会员，JumpFrp感谢您的支持！。`,
            positiveText: '确定',
            onPositiveClick: () => {
...
async function handlePay(ttype: string) {
    ...
    try {
        const response = await axios.get(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/api/pay`, {
            params: {
...
const handlePurchase = async () => {
    loading.value = true;
    try {
        const response = await axios.get(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/api/jfsc`, {
            params: {
                usertoken: userInfo?.usertoken,
        ...
        if (data?.status === 'success') {
            ...
            dialog.success({
                ...
                positiveText: '确认',
                onPositiveClick: () => {
                    message.success('再次感谢您对JumpFrp的支持！');
                },
            });
        } else {
            message.error(data?.message);
...
const handleUpgrade = async () => {
    loading.value = true;
    try {
        const response = await axios.get(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/api/tcsj`, {
            params: {
                usertoken: userInfo?.usertoken,
