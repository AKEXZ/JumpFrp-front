        ...
        <n-grid-item :span="3">
            <n-card title="常见教程">
                ...
                <n-p>
                    ...
                    即可。<br />
                    web服务可以通过在nginx配置中启用"Proxy Protocol"的解析并将结果设置在"X-Real-IP"这个Header中就可以在自己的Web服务中通过 X-Real-IP 获取到用户的真实 IP。<br />
                    操作：<br />
                    打开nginx网站的配置文件，将
...
const allTunnels = ref<{ name: string; node: string }[]>([]);
...
const getTunnelList = async () => {
    try {
        ...
        allTunnels.value = tunnels.map((t: Tunnel) => ({ name: t.name, node: t.node }));
        const nodes = Array.from(new Set(tunnels.map((t: Tunnel) => t.node)));
        nodeOptions.value = nodes.map((node: string) => ({ label: node, value: node }));
    } catch (error) {
...
watch(nodeValue, (newNode: string | null) => {
    if (newNode) {
        ...
        tunnelOptions.value = allTunnels.value.filter((tunnel) => tunnel.node === newNode).map((tunnel) => ({ label: tunnel.name, value: tunnel.name }));
    } else {
        tunnelOptions.value = [];
    }
});
...
const getConfigFile = async () => {
    ...
    try {
        const params: { token: string | undefined; node: string | null; tunnel_names?: string } = { token: userInfo?.usertoken, node: nodeValue.value };
...
const copyToClipboard = () => {
    navigator.clipboard.writeText(tunnelConfig.value).then(() => { message.success('配置文件复制成功'); }).catch((err) => { console.error('复制配置文件失败:', err); message.error('配置文件复制失败'); });
};
...
const handleCopy = (type: string) => {
    ...
    navigator.clipboard.writeText(content).then(() => message.success('复制成功')).catch(() => message.error('复制失败'));
};
...
<style scoped>
/* ...existing code from ChmlFrp-Panel-v3 ConfigPage.vue style部分... */
</style>
