    ...
    <n-card>
        <div style="display: flex; align-items: center">
            <div style="margin-left: 10px">
                <n-h1 prefix="bar">JumpFrp-下载中心</n-h1>
                <div>
                    <n-p>
                        此处下载的程序为JumpFrp核心程序，需通过终端启动，无操作UI，全平台兼容，bug最少。
                        <!-- 除了这些，您还可以前往菜单的
...
onMounted(async () => {
    try {
        const response = await axios.get<ApiResponse>(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/api/dw`);
