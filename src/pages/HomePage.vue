        ...
        <n-grid style="margin-top: 15px" cols="1 s:5" responsive="screen" :x-gap="15" :y-gap="20">
            <n-gi :span="3">
                ...
                <n-card style="margin-top: 15px">
                    <n-result status="success" title="JumpFrp - Panel v3.0" description="简约 大气 开源">
            ...
            <n-gi :span="2">
                <n-card>
                    ...
                    <n-alert
                        ...
                        @click="goToUserPage"
                    >
                        根据相关法律规定，不实名则无法使用JumpFrp提供的服务，点击此提示可前往个人中心免费实名，我们允许未成年实名。
                    </n-alert>
                ...
                <n-card style="margin-top: 16px" title="常见问题">
                    <n-collapse style="margin-top: 20px">
                        <n-collapse-item title="数据安全" name="1">
                            <div>
                                <n-p>
                                    使用jumpfrp映射远程桌面、数据库等，映射后会暴露在公网环境。您应该注意您的数据安全，设置安全的密码。因此类问题造成的锁机·重要文件被盗，我们概不负责。
                                </n-p>
                                <n-p>
                        ...
                        <n-collapse-item title="关于带宽" name="6">
                            <div>
                                JumpFrp的带宽分为境内、境外两种。境外节点的带宽速度为境内的四倍。假如您的境内带宽为8m，境外带宽则为8*4=32m。
                                <br />
                                我们带宽的计量单位为mbps，计算方式：8mbps=1MiB/s。此外，JumpFrp的带宽为共享峰值带宽，不能保证高峰期能跑满应有的速度。
                                <br />
...
const closeDialog = () => {
    ...
    countdown.value = 10;
    message.info(
        () => `您后续可以从 菜单的"其他信息->帮助文档" 查看所有有关JumpFrp的教程！（${countdown.value}秒后关闭）`,
        {
            duration: 10000,
            onClose: () => {
                clearInterval(timer);
...
const qiandaoinfo = async () => {
    ...
    try {
        const response = await axios.get(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/qdxx`, {
...
const GoToQqGroup = (Link: string, qq: number) => {
    dialog.warning({
        title: '警告',
        content:
            '进群提问题之前，一定要携带完整的报错、截图。例如Frpc运行截图，配置文件、隧道截图。并且请给Token打码。如果无法跳转加入QQ群，请通过此群号进入：' +
            qq,
        positiveText: '我知道了',
        negativeText: '取消',
        onPositiveClick: () => {
...
const trafficInfo = async () => {
    ...
    try {
        const response = await axios.get(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/flow_zong`, {
