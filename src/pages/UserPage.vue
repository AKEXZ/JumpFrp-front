            ...
            <n-gi :span="3">
                <n-card title="消息">
                    ...
                    <n-alert v-if="isTermExpiringSoon" title="您的会员即将到期" type="info" style="margin-bottom: 10px">
                        您的JumpFrp{{ userInfo?.usergroup }}将于 {{ userInfo?.term }} 到期，剩余
...
const giftcode = async () => {
    ...
    try {
        const response = await axios.get(
            `${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/giftcode`,
            { params: { usertoken: userInfo?.usertoken, userid: userInfo?.id, giftcode: exchangeCodeModel.value.exchangeCode } }
        );
...
const realNameHandleValidateButtonClick = async () => {
    ...
    try {
        ...
        const response = await axios.post(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000'}/realname`, formData, {
            headers: {
...
const copyAndToggleContent = () => {
    navigator.clipboard
        .writeText(userInfo?.usertoken || 'JumpFrp账户Token复制失败')
        .then(() => {
